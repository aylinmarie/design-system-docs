...
**Gotcha: bare text/JSX in flow position silently gets wrapped in `<p>`.** This isn't just a Callout thing — *any* hand-written block-level JSX (a `<Flex>`/`<Box>`/`<a>` written on its own lines) whose only content is loose text or an inline element (even a `<span>`) gets that content auto-wrapped in a markdown paragraph node before our `h2`/`p` → Radix mapping runs, which then silently injects `size="3" mb="3"` margin into it — e.g. this broke the CTA buttons on `Introduction.mdx` (extra bottom margin baked into the button) until they were rewritten as a `{(<Flex>...</Flex>)}` JS expression. **The fix is to wrap custom hand-written JSX blocks in a `{(...)}` expression** — content inside a JS expression is parsed by the JS/JSX grammar, not remark, so it's never auto-wrapped. This is exactly what already happens naturally for `.map()`-driven demo widgets (`{items.map(...)}`), which is why those never hit this bug — only hand-written static JSX blocks (no `.map`) are at risk. When adding a new one, check the rendered DOM for a stray `<p class="rt-Text ...">` you didn't write.
