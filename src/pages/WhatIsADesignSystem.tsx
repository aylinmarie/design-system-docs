  ...
  return (
    <>
      ...
      <article className="prose">
        ...
        <p>
          I think about design systems in three layers, each building on the one below:
        </p>

        {/* Layered diagram */}
        <div className="my-6 rounded-xl border border-gray-100 overflow-hidden">
          {[
            { label: 'Patterns & Guidance', sublabel: 'How components compose into flows', color: '#7c3aed', textColor: 'white', opacity: 1 },
            { label: 'Components', sublabel: 'Reusable UI building blocks', color: '#7c3aed', textColor: 'white', opacity: 0.7 },
            { label: 'Tokens & Foundations', sublabel: 'The raw decisions: color, type, spacing', color: '#7c3aed', textColor: 'white', opacity: 0.45 },
          ].map((layer, i) => (
            <div
              key={i}
              className="px-6 py-4 flex items-center justify-between"
              style={{ background: `rgba(124, 58, 237, ${layer.opacity * 0.12})`, borderBottom: i < 2 ? '1px solid rgba(124, 58, 237, 0.08)' : '' }}
            >
              <div>
                <p className="font-semibold text-sm text-gray-900 mb-0.5">{layer.label}</p>
        ...
        <Callout type="warning" title="The naming trap">
          Calling your component library "the design system" sets the wrong expectation. It implies the system is done once the components are built. It isn't. The system includes the people, processes, and principles that keep everything aligned.
        </Callout>
