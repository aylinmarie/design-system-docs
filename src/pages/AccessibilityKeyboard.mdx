...
<Table.Root variant="surface">
  <Table.Header>
    <Table.Row>
      <Table.ColumnHeaderCell>Widget</Table.ColumnHeaderCell>
  ...
  <Table.Body>
    {keyboardPatterns.map(pattern => (
      <Table.Row key={pattern.widget}>
        <Table.Cell>{pattern.widget}</Table.Cell>
        <Table.Cell><Text as="span" className="kbd-mono">{pattern.keys}</Text></Table.Cell>
        <Table.Cell><Text color="gray">{pattern.behavior}</Text></Table.Cell>
      </Table.Row>
    ))}
  </Table.Body>
</Table.Root>
